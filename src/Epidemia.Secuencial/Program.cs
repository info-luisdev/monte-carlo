...
internal static class Program
{
    private static void Main(string[] args)
    {
        ...
        Console.WriteLine("=== Simulación Monte-Carlo SIRD — Versión Secuencial ===");
        ...
        for (int r = 0; r < config.Replicas; r++)
        {
            ...
            var result = Simulator.RunReplica(config, seed, threadCount: 1);
            sw.Stop();
    ...
    private static void ExportResults(List<ReplicaResult> results, SimulationConfig config)
    {
        ...
        CsvExporter.ExportAggregated(Path.Combine(outputDir, "secuencial_stats.csv"), results, config.Days);
        CsvExporter.ExportTimeline(Path.Combine(outputDir, "secuencial_replica0.csv"), results[0].Timeline);
